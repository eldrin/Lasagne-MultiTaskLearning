...
def distrubuted_feature_ext(train_id, data_fn, out_path,
                            agg=[np.mean, np.std, np.median, np.amin, np.amax],
                            target='tg', model_path='restuls/'):
    """"""
    print 'Compile Feature Function!...'
    net, mdl, params = load_check_point(train_id, path=model_path)
    f_feat = build_dist_feat_fnc(net, model_path)

    print 'Extracting!...'
    with h5py.File(data_fn, 'r') as hf:
        Z, y, dset, ids = feature(f_feat, hf, params, target, agg)  # feature
